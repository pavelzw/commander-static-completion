...
| Executable subcommands                                                                              | Require `completionDefinition(declaration, standaloneRoot)`. Dispatch metadata stays on the declaration; parser metadata comes from the independent definition. Active ancestor digit flags require positional boundaries. |
...
| `_executableHandler`           | Detect executable declarations and require explicit completion definitions.             |
