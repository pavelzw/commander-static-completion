...
try {
  ...
  const [packed] = JSON.parse(
    runNpm(["pack", "--json", "--pack-destination", temporary], root),
  ) as PackResult[];
  ...
  for (const file of files) {
    ...
    assert.ok(
      /^(dist\/|src\/.*\.ts$|package\.json$|README\.md$|CHANGELOG\.md$|docs\/commander-compatibility\.md$|LICENSE$)/u.test(
        file,
      ),
    ...
    if (file.startsWith("dist/") && file.endsWith(".js")) {
      assert.ok(
        files.has(file.replace(/^dist\//u, "src/").replace(/\.js$/u, ".ts")),
        `No TypeScript source for ${file}`,
      );
    }
  }
  ...
  const commander = JSON.parse(
    readFileSync(join(root, "node_modules/commander/package.json"), "utf8"),
  ) as { version: string };
  ...
  for (const file of files) {
    ...
    assert.ok(
      existsSync(resolve(dirname(mapPath), map.file)),
      `Map points to missing output: ${file}`,
    );
    ...
    for (const source of map.sources) {
      ...
      assert.ok(
        files.has(path.split(sep).join("/")),
        `Map source is not packaged: ${file} -> ${source}`,
      );
      assert.ok(existsSync(resolved), `Missing map source: ${source}`);
    }
  }
  assert.ok(
    files.has("dist/index.js.map") && files.has("dist/index.d.ts.map"),
    "JavaScript and declaration maps must be published.",
  );
