...
test('fish: native filesystem completions, including assignments', () => {
  ...
  try {
    ...
    assert.deepEqual(complete('fish', ['csc-test-cli', 'deploy', '--config', 'two'], cwd).sort(), ['two directories/', 'two words.json']);
    assert.deepEqual(complete('fish', ['csc-test-cli', 'deploy', '--config=two'], cwd).sort(), ['--config=two directories/', '--config=two words.json']);
