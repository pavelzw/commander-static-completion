...
for (const shell of shells) {
  test(`${shell}: shared completion behavior`, () => {
    for (const [name, words, expected] of cases) {
      assert.deepEqual(complete(shell, fixture(), ['csc-test-cli', ...words]).sort(), [...expected].sort(), name);
    }
  });

  test(`${shell}: visibility and literal choices`, () => {
    const root = complete(shell, fixture(), ['csc-test-cli', '']);
    for (const name of ['deploy', 'd', 'remote', 'help', '--help', '--version']) assert.ok(root.includes(name), name);
    assert.ok(!root.includes('internal') && !root.includes('--secret'));
    assert.deepEqual(complete(shell, fixture(), ['csc-test-cli', 'deploy', '--target', '']).sort(),
...
function probeProgram({ positional = false, passThrough = false, digit = false, leafDigit = false, shadow = false } = {}, observed = []) {
  const record = label => value => {
    if (value === probe) observed.push(`${probe}${label}`);
    return value;
  };
  const option = (flags, label) => new Option(flags).choices([`${probe}${label}`]).argParser(record(label));
  const args = (command, label) => command
    .addArgument(new Argument('[first]').choices([`${probe}${label}-first`]).argParser(record(`${label}-first`)))
    .addArgument(new Argument('[rest...]').choices([`${probe}${label}-rest`]).argParser((value, previous = []) => {
      record(`${label}-rest`)(value);
      return [...previous, value];
    }))
    .action(() => {});
  const root = new Command('csc-test-cli').exitOverride().configureOutput({ writeErr() {}, writeOut() {} });
...
const probes = [
  ...
  ['parent value has priority over child variadic values', {}, ['run', '--many', 'one', '--global']],
  ...
  ['local flags take priority in positional mode', { shadow: true, positional: true }, ['run', '--shared']],
...
for (const shell of shells) {
  test(`${shell}: cursor value ownership agrees with Commander`, () => {
    for (const [name, configuration, words] of probes) {
      ...
      const suggestions = complete(shell, probeProgram(configuration), ['csc-test-cli', ...words, probe]);
  ...
  test(`${shell}: positional and pass-through option visibility`, () => {
    ...
    assert.deepEqual(complete(shell, probeProgram({ positional: true }), ['csc-test-cli', 'run', '--g']), []);
    assert.deepEqual(complete(shell, probeProgram({ passThrough: true }), ['csc-test-cli', 'run', 'first', '--v']), []);
    assert.ok(complete(shell, probeProgram({ passThrough: true }), ['csc-test-cli', 'run', '--v']).includes('--value'));
  });
  ...
  test(`${shell}: nested option scopes and variadic command arguments`, () => {
    ...
    assert.deepEqual(complete(shell, root, ['csc-test-cli', 'group', 'leaf', '--root']), ['--root-flag']);
    assert.deepEqual(complete(shell, root, ['csc-test-cli', 'group', 'leaf', '--group']), []);

    const positionalRoot = new Command('csc-test-cli').enablePositionalOptions().option('--root-flag');
    const nonpositionalGroup = positionalRoot.command('group').enablePositionalOptions(false).option('--group-flag');
    nonpositionalGroup.command('leaf').option('--leaf-flag');
    assert.deepEqual(complete(shell, positionalRoot, ['csc-test-cli', 'group', 'leaf', '--root']), []);
    assert.deepEqual(complete(shell, positionalRoot, ['csc-test-cli', 'group', 'leaf', '--group']), ['--group-flag']);
    ...
    const wrapper = new Command('csc-test-cli').passThroughOptions()
      .option('--owned').argument('[args...]');
