...
function insert(cwd, input) {
  ...
  writeFileSync(join(cwd, 'setup.bash'), `
...
`;
  const result = spawnSync(executables.zsh, ['-f'], {
    input: driver, cwd, encoding: 'utf8', timeout: 15000,
    env: { ...process.env, TERM: 'xterm', PS1: 'CSC_PROMPT> ', INPUTRC: '/dev/null', HISTFILE: '/dev/null' },
  });
  assert.equal(result.status, 0, `${JSON.stringify(input)}: ${result.error ?? ''}\n${result.stderr}\n${result.stdout}`);
  assert.equal(existsSync(join(cwd, 'invoked')), false);
  assert.equal(existsSync(join(cwd, 'PWNED')), false);
  return readFileSync(join(cwd, 'result'), 'utf8').replace(/^\s*\d+\s+#/, '').replace(/\n$/, '');
}
...
test('bash: native Readline insertion', async t => {
  ...
  try {
    ...
    const cases = [
      ...
      ['middle of line', 'deploy --target pr --no-cache' + '\x02'.repeat(11) + '\t', 'deploy --target production --no-cache'],
    ];
    for (const [name, input, expected] of cases) {
      await t.test(name, () => assert.equal(insert(cwd, 'csc-test-cli ' + input), 'csc-test-cli ' + expected));
    }
