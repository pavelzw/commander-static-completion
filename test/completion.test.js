...
test("option values, aliases, assignments and Bash word breaks", () => {
  ...
  assert.deepEqual(complete(fixture(), ["csc-test-cli", "deploy", "--target=pr"]), [
    "--target=production",
  ]);
...
test("file and directory hints use the shell filesystem", () => {
  ...
  try {
    ...
    assert.deepEqual(
      complete(fixture(), ["csc-test-cli", "deploy", "--config", "two"], { cwd: dir }),
      ["two words.json"],
    );
    const program = new Command("csc-test-cli").addArgument(
      completionHint(new Argument("[dir]"), { kind: "directory" }),
    );
...
test("unsupported configurations and invalid inputs produce diagnostics", () => {
  ...
  assert.throws(
    () => completionHint(new Option("--foo"), { kind: "choices", values: [1] }),
    /strings/,
  );
});
